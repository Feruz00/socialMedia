...
export default function ConversationModal({setShow,  filter}){
    ...
    async function forcreate(){
        await axios({
            ...
            url: config + '/api/v1/conversations/findOrCreate'
        }).then( res=>{
    ...
    useEffect( ()=>{
        ...
        const getContacts = async ()=>{
            try {
                const res = await axios.get(`${config}/api/v1/friends/users/all`, {withCredentials: true})
                let all = res.data;
                if( filter.length > 0 ){
                    all = all.filter( i=>{
        ...
        <Modal.Body>
            {
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    <Button type="submit" className="rounded-0" disabled = {hloading || selectedContacts.length===0} > 
